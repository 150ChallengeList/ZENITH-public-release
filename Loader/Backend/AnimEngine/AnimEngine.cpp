...
void AnimEngine::renderHud(void* state) {
	...
	for (auto& [label, controller] : this->controllers) {
		...
		float newHoverState = controller->_hoverState ?
			static_cast<float>(min((curTime - controller->_hoverTime) * animMul, 1.0)) : // 0.0 -> 1.0
			static_cast<float>(max((controller->_hoverTime + this->animLength - curTime) * animMul, 0.0));

		controller->tHover = newHoverState;

		float newClickState = controller->_clickState ?
			static_cast<float>(min((curTime - controller->_clickTime) * animMul, 1.0)) : // 0.0 -> 1.0
			static_cast<float>(max((controller->_clickTime + this->animLength - curTime) * animMul, 0.0)); // 1.0 -> 0.0
